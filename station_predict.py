...
def evaluate(df_scored: pd.DataFrame, risk_threshold: float = None, lead_hours: int = 12):
    ...
    Returns a dict of metrics AND a "report_text" string (the same content
    that gets printed), so the caller can write it straight to a file.
    """
    lines = []
    ...
    log(f"Average precision (label vs risk_score): {ap:.3f}")
    ...
    log(f"Using risk threshold: {risk_threshold:.4f}")

    false_alarm_rate = (
        valid.loc[valid.label == 0, "risk_score"] > risk_threshold
    ).mean()
    log(f"False alarm rate on normal hours: {false_alarm_rate:.3%}")
...
if __name__ == "__main__":
    ...
    # Save every scored observation (features, label, risk_score) to CSV.
    # This is the file to open in Excel/pandas to dig into individual stations.
    scored_path = os.path.join(OUT_DIR, "scored_results.csv")
    scored.to_csv(scored_path, index=False)
    print(f"\nSaved scored results to {scored_path}")

    # Save a plain-text summary report with the same metrics printed above.
    report_path = os.path.join(OUT_DIR, "evaluation_report.txt")
    with open(report_path, "w") as f:
        f.write("Weather Station Failure Prediction -- Evaluation Report\n")
        f.write("=" * 55 + "\n\n")
    ...
    for _, r in live_station_data.iloc[150:175].iterrows():
        ...
        at_risk = bool(risk is not None and risk > results["threshold"])
        flag = "  <-- AT RISK" if at_risk else ""
        print(f"{r.timestamp}: risk={risk}{flag}")
        live_log_rows.append({"timestamp": r.timestamp, "risk_score": risk, "at_risk": at_risk})

    live_log_path = os.path.join(OUT_DIR, "live_monitor_log.csv")
    pd.DataFrame(live_log_rows).to_csv(live_log_path, index=False)
    print(f"\nSaved live monitor log to {live_log_path}")

    print(f"\nAll outputs written to the '{OUT_DIR}/' folder:")
    print(f"  - {scored_path}")
    print(f"  - {report_path}")
    print(f"  - {live_log_path}")
